...
describe('changeset', function () {
  ...
  describe('Happy Path', function () {
    ...
    describe('PATCH /changeset/{changesetId}', function () {
      it('should return 200 status code and OK body', async function () {
        ...
        expect(await requestSender.postChangeset(app, body)).toHaveStatus(StatusCodes.CREATED);
  ...
  describe('Flow', function () {
    it('should create sync, files, entities, changeset and close it', async function () {
      ...
      expect(await postSync(app, sync)).toHaveStatus(StatusCodes.CREATED);
      ...
      expect(await postFile(app, sync.id as string, file1)).toHaveStatus(StatusCodes.CREATED);
      expect(await postFile(app, sync.id as string, file2)).toHaveStatus(StatusCodes.CREATED);
      ...
      expect(await requestSender.postChangeset(app, changeset1)).toHaveStatus(StatusCodes.CREATED);

      expect(await getLatestSync(app, sync.layerId as number)).toHaveProperty('body.status', Status.IN_PROGRESS);

      expect(await requestSender.postChangeset(app, changeset2)).toHaveStatus(StatusCodes.CREATED);
      ...
      expect(await patchEntities(app, patchBody)).toHaveStatus(StatusCodes.OK);
      expect(await requestSender.putChangeset(app, changeset1.changesetId as string)).toHaveStatus(StatusCodes.OK);

      expect(await getLatestSync(app, sync.layerId as number)).toHaveProperty('body.status', Status.IN_PROGRESS);

      expect(await requestSender.putChangeset(app, changeset2.changesetId as string)).toHaveStatus(StatusCodes.OK);
  ...
  describe('Flow with mixed synced and not synced entities', function () {
    it('should create a sync with not synced entity that should complete the file and the sync', async function () {
      ...
      expect(await postFile(app, sync.id as string, file)).toHaveStatus(StatusCodes.CREATED);
      ...
      expect(await postEntityBulk(app, file.fileId as string, fileEntities)).toHaveStatus(StatusCodes.CREATED);
      ...
      expect(await requestSender.postChangeset(app, changeset)).toHaveStatus(StatusCodes.CREATED);
      ...
      expect(await requestSender.putChangeset(app, changeset.changesetId as string)).toHaveStatus(StatusCodes.OK);

      expect(await getLatestSync(app, sync.layerId as number)).toHaveProperty('body.status', Status.IN_PROGRESS);

      // patch the other entity as not synced should complete the whole sync
      expect(await patchEntity(app, file.fileId as string, notSyncedEntity.entityId as string, { status: EntityStatus.NOT_SYNCED })).toHaveStatus(
        StatusCodes.OK
      );
    ...
    it('should create a sync with not synced entity that should only complete the file but not the whole sync', async function () {
      ...
      expect(await postEntityBulk(app, file1.fileId as string, file1Entities)).toHaveStatus(StatusCodes.CREATED);
      expect(await postEntity(app, file2.fileId as string, file2Entity)).toHaveStatus(StatusCodes.CREATED);
